...
@click.command()
@click.option('--ignore-errors/--no-ignore-errors',
  'ignore_errors',
  help='Whether errors in the conformance/validity of the input EDID will be ignored or will prevent an output file from being generated.',
  show_default=True,
  default=False)
@click.argument('edid_file',
  type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True))
def edid2json(ignore_errors, edid_file):
  """Takes in binary EDID_FILE, parses it into a JSON object, and prints it to the stdout."""
...
def json2edid(json_file, edid_file):
  """Loads in JSON_FILE describing an EDID and writes the corresponding binary EDID_FILE."""
